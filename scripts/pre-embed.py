...
IMG_PATH = "E:/imagenet_images"
IMG_SHAPE = (224, 224, 3)  # for VGG
# IMG_SHAPE = (299, 299, 3)  # for Xception
OUT_NAME = "../data/big/mcrae-484x100-vgg19.emb"
OUT_DIM = 50
BATCH_SIZE = 16
FILES_ATLEAST = 100
...
for i in range(0, len(fnames), BATCH_SIZE):
    ...
    with open(OUT_NAME, "a") as out:
        for j in range(i, min(i + BATCH_SIZE, len(fnames))):
            print(fnames[j].replace(" ", "\\ "), " ".join([f"{n:4e}" for n in output[j%BATCH_SIZE]]), file=out)
